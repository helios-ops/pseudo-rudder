		...
		)
{
     ...
     while (!feof(f)) 
     {
      ...
      /* ----- Set up args for LibVEX_Translate ----- */
          ...
          printf ( " %6.2f ... %u\n", 
               	 (double)trans_used / (double)vge.len[0], 
               	 sum 
                    );
                     		    ...
                     		    )
{
     ...
     for (i = 0; i <  bb_in->stmts_used; i++) 
     {
	   ...
	   switch (st->tag) 
	   {
	        case Ist_WrTmp:
            	      assign( bb, 
				     findShadowTmp(&mce, st->Ist.WrTmp.tmp), 
        	 ...
        	 case Ist_Store:
	             do_shadow_STle( &mce, st->Ist.Store.addr, 
				 			0/* addr bias */,
        		                            st->Ist.Store.data,
                       			       NULL /* shadow data */ 
                       			     );
