...
Stmt *translate_stmt( IRStmt *stmt, IRSB *irbb, vector<Stmt *> *irout )
{
    ...
    switch ( stmt->tag )
    {
        ...
        case Ist_Store:
            ...
            break;
        case Ist_Dirty:
            result = new ExpStmt(new Unknown(uTag + "Dirty"));
            break;
        case Ist_MFence:
...
vector<Stmt *> *translate_irbb( Instruction *inst, IRSB *irbb )
{
    ...
    for(int i = 0; i < irbb->tyenv->types_used; i++){
      ...
      reg_t typ = IRType_to_reg_type(ty);
      string name = "T_" + int_to_str(get_type_size(typ)) + "t" +
	int_to_str(i);
...
// HHui added at Jan 8th, 2012 
/* NOTE: As the code segment always contain some data part, causing the original IR-lift util 
   fail to work, I try to combine IDA-analysis results to ignore those data part here.
 */
/* ================================================================================================= */
#ifndef H_IRLIFT_COMBINE_IDA_TO_INGORE_DATA_IN_CODE
    #define H_IRLIFT_COMBINE_IDA_TO_INGORE_DATA_IN_CODE
        
    typedef struct H_data_range_entry
    {
	uint32_t startVA;
	uint32_t endVA;
	uint32_t match_count;
	struct H_data_range_entry * next;
    }H_data_range_entry_t, *PH_data_range_entry_t;
    
    typedef struct H_data_range_list
    {
	H_data_range_entry_t * head;
	H_data_range_entry_t * end;
	int  count;
    }H_data_range_list_t, *PH_data_range_list_t;

    H_data_range_list_t h_drlist;
    int h_ir_logfd     = -1;
    int h_ir_initlogfd = -1;

    static void H_data_range_list_init( )
    {
	h_drlist.head  = NULL;
	h_drlist.end   = NULL;
	h_drlist.count = 0;
        ...
        umask(0);
        h_ir_logfd = open( "./ir_logfile",
	         	  (O_CREAT | O_RDWR),
	       	 	  (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH)
	       		 );

	h_ir_initlogfd = open( "./ir_init_logfile",
	         	       (O_CREAT | O_RDWR),
	       	 	       (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH)
	       		     );
    }// end of H_data_range_list_init( )

    void H_data_range_list_addentry( uint32_t startVA,
				     uint32_t endVA
				   )
    {
	H_data_range_entry_t * entry = (H_data_range_entry_t *)malloc(sizeof(H_data_range_entry_t));
	char h_irbuf[50];
	int  h_ircount = 0;
	...
	{
	    (h_drlist.end)->next = entry;
	    h_drlist.end	 = entry;
	}// end of if(h_drlist.head)

	h_drlist.count = h_drlist.count + 1;

	if(h_ir_initlogfd > 0)
	{
	    h_ircount = sprintf( h_irbuf,
				 "add range : [%x - %x]\n",
				 startVA,
				 endVA
			       );
	    write( h_ir_initlogfd,
		   h_irbuf,
		   h_ircount		   
		 );
	}// end of if(h_ir_initlogfd)

    }// end of H_data_range_list_addentry( )

    static void H_data_range_list_delete( )
    {
	H_data_range_entry_t * entry = NULL;
	
	while(h_drlist.head)
	{
	    entry = (h_drlist.head)->next;
	...
	if(h_ir_initlogfd > 0)
	{
	    close(h_ir_initlogfd);
	}// end of if(h_ir_initlogfd)

	h_ir_logfd     = -1;
	h_ir_initlogfd = -1;
    }// end of H_data_range_list_delete( )


    void H_dada_range_list_delentry(uint32_t startVA)
    {
	H_data_range_entry_t * pre_entry = h_drlist.head;
	H_data_range_entry_t * entry	 = NULL;

	if( (h_drlist.head)->startVA == startVA )
	{
	    h_drlist.head = (h_drlist.head)->next;
	    free(pre_entry);
	    return;
	}// end of if((h_drlist.head))

	entry = pre_entry->next;
	do
	{
	    if(entry->startVA == startVA) 
	    {
		pre_entry->next = entry->next;
		free(entry);
		break;
	    }// end of if(entry->startVA)

	    pre_entry = entry;
	    entry     = entry->next;
	}while(entry != NULL);
    }// end of H_dada_range_list_delentry( )


    int isInDataRange(uint32_t va, uint32_t length)
    {
	H_data_range_entry_t * entry = h_drlist.head;
	...
	while(entry != NULL)
	{
	    if( (va >= entry->startVA) && (va < entry->endVA) )
	    {
		entry->match_count = entry->match_count + length;
		match_offset = entry->match_count;
		range_length = entry->endVA - entry->startVA;

		if(match_offset == range_length)
		{
		    logcount = sprintf( logbuf,
				        "match_offset = %x, range_length = %x, entry->startVA = %x\n",
				        match_offset, 
				        range_length,
				        entry->startVA
				      );
		    ...
		    // H_dada_range_list_delentry(entry->startVA);
		}// end of if(entry->match_count)
	...
	return 0;
    }// end of isInDataRange( )


    static uint32_t h_a2i(char * str)
    {
        uint32_t data = 0;
    ...
    void HexA2H( char *     H_buffer,
		 uint32_t * startVA,
		 uint32_t * endVA
	       )
    {
	H_buffer[0 + 8] = '\0';
	*startVA    = h_a2i(H_buffer);

	H_buffer[9 + 8] = '\0';
	*endVA      = h_a2i(H_buffer + 9);
    }// end of HexA2H( )

    
    #ifndef H_DBG_CARE_TEST
	#define H_DBG_CARE_TEST
    #endif 

    #ifdef H_DBG_CARE_TEST
        int H_dbg_care(uint32_t addr, uint32_t dst_addr)
        {
	    static int H_dbg_count = 0;
	
            if((uint32_t)addr == (uint32_t)(dst_addr))
            {
	        H_dbg_count = H_dbg_count + 1;
	    }// end of if(addr)
...
#ifdef H_IRLIFT_COMBINE_IDA_TO_INGORE_DATA_IN_CODE  

    #ifdef H_DBG_CARE_TEST  
        H_dbg_care(addr, 0x40a2b0);
    #endif

    H_len = inst->length;

    if(isInDataRange(addr, H_len))
    {
	continue;
    }// end of if(isInDataRange)
#endif
...
/* --------------------------------------------------------------------------------------- */
    ...
    H_idx   = H_fname.rfind("/");
    H_fname = H_fname.substr(H_idx + 1);   // image-filename
    H_fname = "./IDA_analysis/" + H_fname + "_noninsn_area";
    strcpy(tbuf, (char *)( H_fname.c_str( ) ));

    H_data_range_list_delete( );
    H_data_range_list_init( );

    umask(0);
    H_fd = open( H_fname.c_str( ),
	         (O_CREAT | O_RDWR),
	       	 (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH)
	       );
    if(H_fd >= 0)
    {
	H_count = read( (int)H_fd,
		        (void *)H_buffer,
		        (size_t)(8 + 8 + 1 + 1) 
		      );
	...
	while(H_count > 0)
	{
	    for(H_i = 0; H_i < 18; H_i = H_i + 1)
	    {
		if(H_buffer[H_i] == ' ')
		{
		    H_buffer[H_i] = '0';
		}// end of if(H_buffer[H_i])
	    }// end of for{H_i}
	    ...
	    #ifdef H_DBG_CARE_TEST  
		H_dbg_care(startVA, 0x40a2b0);
	    #endif

	    /*
	    #ifdef H_DBG_CARE_TEST
		H_dbg_care(startVA, 0x32b749f);		
	    #endif
	    */

	    H_data_range_list_addentry( startVA,
					endVA
				      );
...
vector<vine_block_t *>
generate_vine_ir( asm_program_t *	      prog, 
				   ...
				   )
{
    ...
    for ( int i = 0; i < vblocksize; i++ )
    {
	  ...
	  if(is_debug_on("vex")) 
	  {
	       // using stdout because ppIRSB prints to stdout
	       // FIXME: memory leak
	       if(block->inst)
	       {
	            printf( "vex: asm: %s", 
