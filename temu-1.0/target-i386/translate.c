...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
...
#ifdef H_DEBUG_TEST
    if( ( temu_plugin != NULL ) && 
	( (uint32_t)(temu_plugin->dbg_interested_eip) != 0 
	)
      )
    {
        if( (uint32_t)pc_start == (uint32_t)(temu_plugin->dbg_interested_eip) )
        {
	    term_printf( "disas_insn( ) : eip = 0x%x\n",
		         (uint32_t)pc_start
		       );
        }// end of if(pc_start)
 ...
 reswitch:
	...
	/*
	if( ( temu_plugin != NULL ) &&
	    ( temu_plugin->HHui_conjmp != NULL )
	  )
	{
	    temu_plugin->HHui_conjmp(op);
	}// end of if( )
	*/
	/* ------------------------------------------------------------------------------------------ */ // HHui added !
...
static void optimize_flags(uint16_t *opc_buf, int opc_buf_len)
{
	...
	/*
	#ifdef H_DEBUG_TEST
	if(op == INDEX_op_testl_T0_T1_cc)
	{
	    term_printf( "op = INDEX_op_testl_T0_T1_cc(%x)\n",
			 (uint32_t)INDEX_op_testl_T0_T1_cc
		       );
	}// end of if(op)
	#endif
	*/
