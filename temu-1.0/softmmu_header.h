...
static inline RES_TYPE glue(glue(ld, USUFFIX), MEMSUFFIX)(target_ulong ptr)
{
	...
	/*
	if( ( temu_plugin != NULL ) && 
	    ( temu_plugin->HHui_memory_access != NULL )
	  )
	{
	    temu_plugin->HHui_memory_access( addr,
					     DATA_SIZE,
					     0
					   );
	}// end of if( )
...
#endif
            ( temu_plugin->HHui_write_memory_access != NULL )
	  )
	{
	    temu_plugin->HHui_write_memory_access( addr, 
				  	           DATA_SIZE
			   	     		 );
	}// end of if( )
