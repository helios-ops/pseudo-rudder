...
/* HHui added util for EFLAGS write ! */
/* ------------------------------------------------------------------------------------ */
		      ...
		      )
{
     ...
     cpu_single_env->eflags = ( cpu_single_env->eflags ) & mask;
     cpu_single_env->eflags = ( cpu_single_env->eflags ) | value;
    
}// end of TEMU_EFLAGS_write( )
/* ------------------------------------------------------------------------------------ */
...
uint32_t h_dbg_TEMU_memory_rd_4_bytes(uint32_t vaddr, int len)
{
    char buf[4];
    TEMU_read_mem(vaddr, 
		  len, 
...
void TEMU_update_cr3( )
{
  ...
  else
  {
      ...
      // HHui added for multiple paths' constraints exploration ! --- trying snapshots' switches 
      /* --------------------------------------------------------------------------------- */
/*
      if( (temu_plugin->monitored_cr3 != 0) && ( temu_plugin->monproc_terminated == 1)
	)
      {
          temu_plugin->BFS_restore_HVM_state_from_snapshot( );
      }// end of if( )

  */  
      /* --------------------------------------------------------------------------------- */ // HHui added for multiple paths' constraints exploration !

  }// end of if( )


}// end of TEMU_update_cr3( )
...
// HHui added at May 23rd, 2011
/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
    term_printf( "TEMU_loadvm( ) is called ! --- loadvm_args[0]=%s\n",
		 loadvm_args[0]
	       );    
/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
