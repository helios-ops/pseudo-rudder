                           ...
                           uint32_t base, uint32_t size)
{
  ...
  for (iter = process_list.begin(); iter!=process_list.end(); iter++) {
	...
	/*
	term_printf( "procmod_insert_modinfo( ) --- main module name %s is the proc's name : %s",
		     name,
	     	     (proc->name).c_str( )
		   );
	*/
...
int procmod_createproc(uint32_t pid, uint32_t cr3, const char *name)
{
  ...
  /*
  term_printf( "HHui procmod_createproc( ) : %s\n", 
		name
	     );
  */
...
//    long taskaddr = 0xC033C300; 
  ...
  do {
    ...
    /*
    term_printf( "update_proc( ) -- name = %s\n",
		 comm
	       );
    */
...
// HHui Added at April 22nd, 2011
/* ---------------------------------------------------------------------------------------------------- */
int H_find_procnamebyid(uint32_t pid, char proc_name[])
{
  process_info_t *proc;
  list<process_info_t *>::iterator iter;
  ...
  for(iter = process_list.begin(); iter!=process_list.end(); iter++) 
  {
	  ...
	  // name = ( proc->name ).c_str( );
	  
          // *pid = proc->pid;
	  /*
	  term_printf( "H_Find_Process : %s --- length : %d\n", 
		       // name
		       ( proc->name ).c_str( ),
		       strlen( ( proc->name ).c_str( ) )	
		     );
	  */
...
static int procmod_load(QEMUFile * f, void *opaque, int version_id)
{
  ...
  for (i = 0; i < nproc; i++) 
  {
    ...
    if(strcmp(name, "readfile_for3_3.exe") == 0)
    {
        term_printf( "proc restoration : name = %s\n",
		     name
	           );
    }// end of if( )
...
void parse_process(char *log)
{
  ...
  /*
  term_printf( "guest_os msg-parsing : %s\n",
	       log
	     );
  */

  // original code : if (sscanf(log, "P %c %d", &c, &pid) < 2)
  if (sscanf(log, "P %c %d %8x", &c, &pid, &p_eprocess) < 3)
  {
    // term_printf("----\n");
    return;
  }// end of if( )

  // term_printf("11111111\n");
...
/*
    if( (temu_plugin != NULL) && ( temu_plugin->monitored_pid != 0) && ( pid == temu_plugin->monitored_pid )
      )
    {
	term_printf( "my desired EPROCESS is at address 0x%x\n",
		     p_eprocess
		   );
	temu_plugin->p_eprocess = p_eprocess;
    }// end of if( )
*/
...
void parse_module(char *log)
{
  ...
  if (sscanf(log, "M %d %x \"%[^\"]\" %x %x %x", &pid, &cr3, mod, &base, &size, &eproc) < 6) 
  {
    if (sscanf(log, "M %d %x %s %x %x %x", &pid, &cr3, mod, &base, &size, &eproc) < 6)
      return;
  }
  ...
  if( (temu_plugin != NULL) && 
      (temu_plugin->monitored_pid != 0) && 
      (temu_plugin->monitored_pid == pid) && 
      (temu_plugin->p_eprocess == 0)
    )
  {

    term_printf( "module_msg : %s\n",
		 log
	       );

    temu_plugin->p_eprocess    = eproc;
    temu_plugin->monitored_vad = temu_plugin->obtain_vad( (void *)eproc );


    term_printf( "my desired EPROCESS is at address 0x%8x, VAD is at address 0x%8x\n",
		 eproc,
		 temu_plugin->monitored_vad
	       );
