...
// possibly a peek of some related taint-structure ... maybe should be encapsulated as a plugin-option ... 
#include "./sample_plugin/H_taint_record.h"
...
// EFLAGS
/* --------------------------------------------------------------- */
uint8_t * Get_eflags_records( )
{
  return eflags_records;
}

uint32_t * Get_eflags_bitmap( )
{
  return &eflags_bitmap;
}
#endif
/* --------------------------------------------------------------- */
...
// Registers
/* --------------------------------------------------------------- */
uint64_t * Get_regs_bitmap( )
{
  return &regs_bitmap;
}

uint8_t * Get_regs_records( )
{
  return regs_records;
}
...
{
  taint_operand_t h_dst_opnd ;

  if(!TEMU_emulation_started) return;

// no taint has ever been introduced !
#ifdef HHUI_SYMEXE_DISABLE_UNTIL_TAINT_APPEAR
  if(temu_plugin->symexe_enabled_for_taint == 0)
  {
     return;
  }// end of if(temu_plugin->symexe_enabled_for_taint)
#endif
  ...
  if (offset + size <= 64) {
    ...
    if(taint) {
      ...
      /*
      term_printf("taintcheck_reg  ==>  mem 1\n");
      ...
      */
      ...
      /*
      term_printf( "virtual address is %8x, size is %d\n \n", 
		   cpu_single_env->regs[R_A0],
		   size
		 );
      */

      h_dst_opnd.taint = taintcheck_check_virtmem( cpu_single_env->regs[R_A0],  // virtual address
						   size,
					           NULL
						 );
      /*
      term_printf( " taintcheck_check_virtmem( ) finished !\n"
		 );
       */

      // originally clean dst operand ----- Now get the originally concrete value for SYMBOLIC CALCULATION !
      if(h_dst_opnd.taint == 0)
      {
          h_dst_opnd.type = 1; 		// mem
	  h_dst_opnd.size = size;
    	  h_dst_opnd.addr = cpu_single_env->regs[R_A0];
	
	  if( ( temu_plugin != NULL ) && 
	      ( temu_plugin->get_insn_dst_concrete_value != NULL)
	    )
	  {
	      /*
	      term_printf( "dst is a memory --- mem addr = 0x%8x, eip = 0x%8x !\n",
			   cpu_single_env->regs[R_A0],
			   *TEMU_cpu_eip
			 );
	       */

	      temu_plugin->get_insn_dst_concrete_value( &h_dst_opnd );
	  }// end of if( )
      ...
      // HHui Added to keep the symbolic machine state
      if( temu_plugin->before_taint_propagate( ) == 1)
      {
        return;	 
      }// end of if( )
    ...
    /* Now taint the separate 2 parts */
    ...
    if(taint) {
      ...
      /* ------------------------------------------------------------------------------------------------------------- */
      // term_printf("taintcheck_reg  ==>  mem 2\n");

      h_dst_opnd.taint = taintcheck_check_virtmem( cpu_single_env->regs[R_A0],
						   size1,
					           NULL
						 );
      // originally clean dst operand ----- Now get the originally concrete value for SYMBOLIC CALCULATION !
    ...
    if(taint) {
      ...
      /* ------------------------------------------------------------------------------------------------------------- */
      // term_printf("taintcheck_reg  ==>  mem 3\n");

      h_dst_opnd.taint = taintcheck_check_virtmem( cpu_single_env->regs[R_A0] + size1,
						   size2,
					           NULL
						 );
      // originally clean dst operand ----- Now get the originally concrete value for SYMBOLIC CALCULATION !
      ...
      {
          h_dst_opnd.type = 1; 	  // mem
	  h_dst_opnd.size = size2;
    	  h_dst_opnd.addr = cpu_single_env->regs[R_A0] + size1;
...
// no taint has ever been introduced !
  ...
  if(offset + size <= 64) {
    ...
    if (taint) {
      ...
      /* ------------------------------------------------------------------------------------------------------------- */
      /*
      term_printf( "taintcheck_mem  ==>  reg ( regidx = 0x%8x, offset = 0x%8x )\n", 
		   ( regidx / 4 ),
		   ( regidx % 4 )
		 );	
       */

      h_dst_opnd.taint = taintcheck_register_check( ( regidx / 4 ),
						    ( regidx % 4 ),
						    size,
					            NULL
						  );
      ...
      if(h_dst_opnd.taint == 0)
      {
          h_dst_opnd.type = 0;       // register
	  h_dst_opnd.size = size;
    	  h_dst_opnd.addr = regidx;  
    ...
    if (taint) {
      ...
      /* ------------------------------------------------------------------------------------------------------------- */
      // term_printf("taintcheck_mem  ==>  reg\n");
      /*
      term_printf( "taintcheck_mem  ==>  reg ( regidx = 0x%8x, offset = 0x%8x )\n", 
		   ( regidx / 4 ),
		   ( regidx % 4 )
    ...
    if (taint) {
      ...
      h_dst_opnd.taint = taintcheck_register_check( ( (regidx+size1) / 4 ),
						    ( (regidx+size1) % 4 ),
						    size2,
					            NULL
						  );
      ...
      if(h_dst_opnd.taint == 0)
      {
          h_dst_opnd.type = 0; 	  // reg
	  h_dst_opnd.size = size2;
    	  h_dst_opnd.addr = regidx+size1;
...
// no taint has ever been introduced !
  ...
  if (offset + size <= 64) {
    ...
    if (taint) {
      ...
      /* ------------------------------------------------------------------------------------------------------------- */
      // term_printf("taintcheck_mem  ==>  reg  ---- nolookup !\n");
      /*
      term_printf( "taintcheck_mem2reg_nolookup --- mem  ==>  reg ( regidx = 0x%8x, offset = 0x%8x )\n", 
		   ( regidx / 4 ),
		   ( regidx % 4 )
		 );
    ...
    if (taint) {
       ...
       */
      h_dst_opnd.taint = taintcheck_register_check( (regidx / 4),  // reg
						    (regidx % 4),  // offset
						    size1,	   // size
					            NULL 	   // records
						  );
	...
	int size)
{
  ...
  /* ------------------------------------------------------------------------------------------------------------- */
  // term_printf("taintcheck_ reg  ==>  reg  ---- internal !\n");

  /*
  term_printf( "reg2reg_internal --- mem  ==>  reg ( regidx = 0x%8x, offset = 0x%8x )\n", 
	       ( dregidx / 4 ),
	       ( dregidx % 4 )
	     );
   */
  h_dst_opnd.taint = taintcheck_register_check( ( dregidx / 4 ),
						( dregidx % 4 ),
					        size,
					        NULL
					      );
...
// no taint has ever been introduced !
  ...
  /* ------------------------------------------------------------------------------------------------------------- */
  // term_printf("taintcheck_fn1reg( )!\n");
  /*
  term_printf( "taintcheck_fn1reg --- fn1  ==>  reg ( regidx = 0x%8x, offset = 0x%8x )\n", 
	       ( regid / 4 ),
	       ( regid % 4 )
	     );
   */
  h_dst_opnd.taint = taintcheck_register_check( (regid / 4),
						(regid % 4),
					        size,
				                NULL
					      );
...
// no taint has ever been introduced !
  ...
  /* ------------------------------------------------------------------------------------------------------------- */
  // term_printf("taintcheck_fn2regs( )!\n");

  /*
  term_printf( "taintcheck_fn1reg --- fn2  ==>  reg ( regidx = 0x%8x, offset = 0x%8x )\n", 
	       ( dregid / 4 ),
	       ( dregid % 4 )
	     );
  */

  h_dst_opnd.taint = taintcheck_register_check( ( dregid / 4 ),
						( dregid % 4 ),
					        size,
				                NULL
					      );
...
#if TAINT_FLAGS
uint32_t HHui_EFLAGS_taint_bitmap;

uint32_t Get_HHui_EFLAGS_taint_bitmap( )
{
  return HHui_EFLAGS_taint_bitmap;
}// end of Get_HHui_EFLAGS_taint_bitmap( )

void __attribute__((fastcall)) 
taintcheck_update_eflags(uint32_t mask)
{
    // HHui added at May 9th, 2011
    /* -------------------------------------------------------------------------- */
    if( (temu_plugin != NULL) &&
#ifdef HHUI_SYMEXE_DISABLE_UNTIL_TAINT_APPEAR
        (temu_plugin->symexe_enabled_for_taint != 0) &&  	
#endif
	(temu_plugin->HHui_modify_EFLAGS_access != NULL)
      )
    {
	/* ++++++++++++++++++++++++++++++++++++++++++++++++++ */
	// HHui_EFLAGS_taint_bitmap = mask;
/*
 ...
 */
	...
	temu_plugin->HHui_modify_EFLAGS_access( mask );
    }// end of if( )
    /* -------------------------------------------------------------------------- */
    // HHui added at May 9th, 2011
...
// possibly a problem
  /* ------------------------------------------------------------------------------------------------------------- */
  // term_printf("taintcheck_logic_T0_T1( )!\n");

  /*
  term_printf( "taintcheck_logic_T0_T1 --- R_T0 ( regidx = 0x%8x, offset = 0x%8x )\n", 
	       ( regid / 4 ),
	       ( regid % 4 )
	     );
  */
  // term_printf( "taintcheck_logic_T0_T1 --- R_T0\n");

  h_dst_opnd.taint = taintcheck_register_check( R_T0,
						0,
					        4,
				                NULL
					      );
...
// no taint has ever been introduced !
  ...
  /* ------------------------------------------------------------------------------------------------------------- */
  // term_printf("taintcheck_fn3regs( )!\n");
  
/*
  term_printf( "taintcheck_fn3regs --- ( regidx = 0x%8x, offset = 0x%8x )\n", 
	       ( dregid / 4 ),
	       ( dregid % 4 )
	     );
 */
                                 ...
                                 uint8_t * records)
{
  ...
  // term_printf("Fuck memory checking !\n");
                              ...
                              uint64_t taint, uint8_t * records)
{
  ...
  (temu_plugin->temu_dbg_util)->h_dbg_register_sym_change(reg);
  #endif
#endif
  ...
  if (taint) 
  {
	...
	if( taint & (1 << i) )
	{
            (temu_plugin->temu_dbg_util)->h_dbg_taint_byte_refered( 
				      ( (H_taint_record_t *)( (uint32_t)records + i * temu_plugin->taint_record_size
						            )
				      )->offset
			   		    		  );
	}// end of if(taint)
...
void taintcheck_taint_virtmem(uint32_t vaddr, uint32_t size, uint64_t taint, void *records) 
{
...
#ifdef H_DEBUG_TEST
  #ifdef H_DBG_CHECK_MONITORED_MACHINE_STATE
  (temu_plugin->temu_dbg_util)->h_dbg_memory_sym_change( vaddr,
							 size
						       );
  #endif
#endif
  ...
  if( (temu_plugin->h_taintcheck_virtmem_hookfn != NULL) &&
      (temu_plugin->is_in_focused_module != 0)
    )
  {
      temu_plugin->h_taintcheck_virtmem_hookfn( vaddr,
						size,
						taint,
						records
					      );
...
{
  /*
  term_printf( "enter ---  taintcheck_jnz_T0_label() !\n"
	     );
   */
  int res = 0;
  ...
  /*
  term_printf( "taintcheck_jnz_T0_label() !"
	     );
  */
  ...
  // HHui added code  at May 7th, 2011 
  // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  // original code temply deleted by HHui at May 7th, 2011 
  /* ========================================================================================== */
  /*
  if (taint_reg_check(R_CC_SRC * 4, 8) && temu_plugin && temu_plugin->cjmp) 
  {
    ...
    term_printf( "taintcheck_jnz_T0_label() --- to call  temu_plugin->cjmp( ) !\n"
	       );
...
static void taintcheck_save(QEMUFile * f, void *opaque)
{
  ...
  // HHui added at May 29th, 2011 for EFLAGS-records' save
  /* ------------------------------------------------------------------------------------ */
#ifdef TAINT_FLAGS
  TEMU_compress_buf(&state, (uint8_t *)&eflags_bitmap, 4 );  
...
static int taintcheck_load(QEMUFile * f, void *opaque, int version_id)
{
  ...
  // HHui added at May 29th, 2011 for EFLAGS-record restoration
  /* ---------------------------------------------------------------------------------------- */
#ifdef TAINT_FLAGS
  TEMU_decompress_buf(&state, (uint8_t *)&eflags_bitmap, 4 );
  ...
  if( (temu_plugin != NULL) &&
      (temu_plugin->hvm_load != NULL)
    )
  {      
      temu_plugin->hvm_load( );
  }// end of if( )  
#endif
  /* ------------------------------------------------------------------------------------- */
